...
ORDERS = gvarstatus("OR_ORDERS") or "(الاوامر|أوامري|م)"
IQTHONPC = gvarstatus("ALIVE_PIC") or "https://telegra.ph/file/7f721fc7856121b3822e7.mp4"
...
IQPIC = gvarstatus("ALIVE_PIC") or "https://telegra.ph/file/7f721fc7856121b3822e7.mp4"
...
async def iq(iqthonevent):
    ...
    ALIVE_TEXT = gvarstatus("ALIVE_TEXT") or "𓆩 𝗐𝖾𝗅𝖼𝗈𝗆𝖾 sᴏᴜʀᴄᴇ ᴍᴀᴛʀɪx ALARB 𓆪"
    IQTHON_IMG = gvarstatus("ALIVE_PIC") or "https://telegra.ph/file/7f721fc7856121b3822e7.mp4"
...
℘︙ : ᴠᴇʀsɪᴏɴ 1.0 .
℘︙ : sᴏᴜʀᴄᴇ ᴍᴀᴛʀɪx : @YY_VP .
℘︙ : sᴜᴘᴘᴏʀᴛ ᴍᴀᴛɪx : @matrix205 .
℘︙ : ᴄʜᴀɴɴᴇʟ ᴡʀɪᴛɪɴɢs : @UXXHT ."""
...
async def amireallyalive(event):
    ...
    IQTHON_ALIVE_TEXT = "❬ ماتريكس العرب - sᴏᴜʀᴄᴇ ᴍᴀᴛʀɪx for arab 𓆩♦️𓆪❭ :"
...
async def _(event):
    ...
    OUTPUT = (f"☸︙  وحـدة المعلومات الخاصه بتنصيبك مع جميع الفارات  لتنصيب سورس  ماتريكس @BU_QH :**\n\n{o}")
...
async def _(event):
    if BOTLOG:
        await event.client.send_message(BOTLOG_CHATID, "**☸︙  𓆩 تم تحديث سورس ماتريكس 𓆪**")
    sandy = await edit_or_reply(event , "☸︙  جـاري تـحديـث سورس ماتريكس  🔄\n🔹 - قـد يستغـرق الأمـر 5 - 10 دقائـق انتـظـر\nلاتقـم بتحـديث أكثـر من 3 مـرات باليـوم" ,)
    try:
    ...
    if vname in vlist:
        ...
        if cmd == "اضف":
            if not vinfo and vname == "ALIVE_TEMPLATE":
                return await edit_delete(event, f"**☸︙  📑 يرجى متابع قناه الفارات تجدها هنا : @BU_QH")
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inlineiqthon(iqthon):
        ...
        query = iqthon.text
        await bot.get_me()
        if query.startswith("المطور") and iqthon.query.user_id == bot.uid:
            buttons = [[Button.url("1- شرح التنصيب", "https://youtu.be/44tYK_yV02Q"), Button.url("2- استخراج ايبيات", "https://my.telegram.org/"),],[Button.url("3- ستخراج تيرمكس", "https://replit.com/@Ahmedyad/YYYBR"), Button.url("4- بوت فاذر", "http://t.me/BotFather"),],[Button.url("5- رابط التنصيب", "https://dashboard.heroku.com/new?template=https://github.com/telethon-Arab/telethohelp"),],[Button.url("المبرمج اسلام 🇪🇬", "https://t.me/TU_PQ"),],[Button.url("المبرمج ماديسون 🇪🇬", "https://t.me/MaDyY_y"),]]
...
async def iq(event):
    await edit_or_reply(event, """✐  - اوامـر سورس ماتريكس تليـثون  . 

       •━═━═━═[『ᴍᴀᴛʀɪx』](https://t.me/YY_VP)═━═━═━• 
℘︙{`.م1`}  اوامر الحساب 1 .
℘︙{`.م2`}  اوامر الحساب 2 .
℘︙{`.م3`}  اوامر الحساب 3 .
℘︙{`.م4`}  اوامر الحساب 4 .
...
℘︙{`.م31`} اوامر الملصقات .
       ...
       •━═━═━═[『ᴍᴀᴛʀɪx』](https://t.me/YY_VP)═━═━═━•
- قم بنسخ الآمر ولصقة لاضهار قائمة الاوامر
لـ روئية المتغيرات ارسل . 
℘︙{ .مساعده }
...
@check_owner
async def inlineiqthon(iqthon):
    text = "**☸︙ قـائمـه الاوامـر :**\n**☸︙ قنـاه السـورس :** @YY_VP\n**☸︙ شـرح اوامـر وفارات السـورس : @BU_QH**\n**☸︙ قناة كتابات : @UXXHT* "
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inlineiqthon(iqthon):
        ...
        query = iqthon.text
        await bot.get_me()
        if query.startswith("(اوامري|الأوامر)") and iqthon.query.user_id == bot.uid:
            buttons = [[Button.inline("اوامر السورس", data="order1"), Button.inline("اوامر الحساب", data="ord1hs"),],[Button.inline("اوامر الكروب", data="ord1G"), Button.inline("اوامر الالعاب", data="ord1pl"),],[Button.inline("اوامر الصيغ", data="ordsag1"), Button.inline("اوامر الاغاني", data="ordSONG"),], [Button.inline("اسم وقتي", data="order13"), Button.inline("اوامر الاعلانات", data="ordahln1"),],[Button.inline("اوامر التسليه", data="order14"),],[Button.inline("الفارات", data="ordvars"),]]
            result = builder.article(title="iqthon",text=help2,buttons=buttons,link_preview=False)
            await iqthon.answer([result] if result else None)
@bot.on(admin_cmd(outgoing=True, pattern="(اوامري|الأوامر)"))
async def repoiqthon(iqthon):
    if iqthon.fwd_from:
        return
    TG_BOT = Config.TG_BOT_USERNAME
    if iqthon.reply_to_msg_id:
        await iqthon.get_reply_message()
    response = await bot.inline_query(TG_BOT, "(اوامري|الأوامر)")
    await response[0].click(iqthon.chat_id)
    await iqthon.delete()
if Config.TG_BOT_USERNAME is not None and tgbot is not None :
    @check_owner
