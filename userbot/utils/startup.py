...
LOGS = logging.getLogger("سورس ماتريكس ✘ \n ")
...
async def startupmessage():
    try:
        if BOTLOG:
            Config.CATUBLOGO = await iqthon.tgbot.send_file(BOTLOG_CHATID, "https://telegra.ph/file/f2b0f74fc233db4f0dd41.mp4", caption="**☸⦙ تـمّ تـحديـث سورس ماتريكس ✘  :**  [ 1.0 ] \n \n**☸⦙ للحصول على اوامر السورس أرسل :** (  `.الاوامر`  ) \n**☸⦙ يحتاج لتفعيل اونلاين أو أرسل :** (  `.اوامري`  )\n\n**☸⦙ القناة الرسمية : @YY_VP **\n**☸⦙ جروب الدعم الرسمي : @matrix205 **\n**☸⦙ فارات وكلايش السورس  : @BU_QH **\n**☸⦙ قناة كتابات :  @UXXHT**",                buttons=[(Button.url("جروب الدعم", "https://t.me/matrix205"),)],            )
    except Exception as e:
        LOGS.error(e)
        return None
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        _, groupid = await create_supergroup(            "مجموعه بوت ماتركس الخاص بك", iqthon, Config.TG_BOT_USERNAME, descript        )
        addgvar("PRIVATE_GROUP_BOT_API_ID", groupid)
        photobt = await iqthon.upload_file(file="bin/IMG_20220215_221601_670.jpg")
    ...
    if PM_LOGGER_GROUP_ID != -100:
        ...
        photobt = await iqthon.upload_file(file="bin/IMG_20220215_221601_670.jpg")
        _, groupid = await create_supergroup(            "جروب تخزين الخاص", iqthon, Config.TG_BOT_USERNAME, descript        )
